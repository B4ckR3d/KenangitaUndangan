...
<head>
    ...
    <link rel="stylesheet" href="<?php echo base_url() ?>/assets/themes/sakura-castle/css/style.css">
...
<div class="thebegining">
    <div class="cover-card">
        <!-- Castle Emblem / Sakura Motif -->
        <img src="<?= base_url() ?>/assets/themes/sakura-castle/img/castle.png" class="castle-cover" alt="Sakura Castle" />

        <p style="font-family: 'Playfair Display', serif; font-size: 11px; letter-spacing: 0.25em; color: #e11d48; text-transform: uppercase; font-weight: 700; margin-bottom: 8px;">
            SAVE THE DATE
        </p>

        <h1 style="font-family: 'Cinzel', serif; font-size: 28px; color: #831843; margin-bottom: 20px; line-height: 1.2;">
        ...
        <div style="padding: 14px 18px; background: #fff5f7; border-radius: 18px; border: 1px solid #fecdd3; margin-bottom: 15px;">
            <p style="font-size: 11px; color: #9f1239; margin-bottom: 4px;">Kepada Yth. Bapak/Ibu/Saudara(i):</p>
            <h3 style="font-family: 'Playfair Display', serif; font-size: 17px; color: #831843; font-weight: 700; margin: 0;">
...
<div id="konten" style="display: none;">
    ...
    <div id="sampul-konten" class="konten">
        <div class="card-sakura text-center">
            <p style="font-family: 'Amiri', serif; font-size: 24px; color: #831843; margin-bottom: 5px;">بِسْمِ اللَّهِ الرَّحْمَٰنِ الرَّحِيمِ</p>
            <p style="font-family: 'Playfair Display', serif; font-size: 12px; letter-spacing: 0.2em; color: #e11d48; font-weight: 700;">THE WEDDING OF</p>
            <h1 style="font-family: 'Cinzel', serif; font-size: 32px; color: #831843; margin: 10px 0;">
                <?php if($posisi_mempelai == 0) echo $nama_panggilan_pria." & ".$nama_panggilan_wanita; else echo $nama_panggilan_wanita." & ".$nama_panggilan_pria;?>
            </h1>
            <p style="font-size: 13px; color: #881337; max-width: 480px; margin: 15px auto; font-style: italic;">
                "Dan di antara tanda-tanda (kebesaran)-Nya ialah Dia menciptakan pasangan-pasangan untukmu dari jenismu sendiri, agar kamu cenderung dan merasa tenteram kepadanya, dan Dia menjadikan di antaramu rasa kasih dan sayang."
            </p>
            <span style="font-size: 11px; color: #e11d48; font-weight: 700;">(QS. Ar-Rum: 21)</span>
        </div>
    </div>
        ...
        <div class="card-sakura text-center">
            ...
            <div style="margin-bottom: 25px;">
                ...
                <div style="margin-top: 8px;">
                    <span style="display:inline-flex; align-items:center; gap:6px; background:#fff5f7; color:#e11d48; padding:4px 14px; border-radius:50px; font-size:11px; font-weight:600; border:1px solid #fecdd3;">
            ...
            <table class="tb-acara">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <th class="tb-anu-acara">:</th>
                        <th class="tb-isi-acara"><?= $data->tempat_acara ?><br><small style="color:#881337;"><?= $data->alamat_acara ?></small></th>
                    </tr>
                </tbody>
            </table>
            <?php } ?>
        ...
        <div class="card-sakura text-center">
            ...
            <ul class="list-group list-group-flush" style="text-align: left;">
                ...
                <li class="list-group-item">
                    <b style="color: #831843; font-family: 'Playfair Display', serif; font-size: 15px;"><?= $row->nama_bank ?></b><br>
                    <span id="norek<?= $i ?>" style="font-size: 16px; font-weight: bold; color: #e11d48;"><?= $row->no_rekening ?></span>
                    <button class="clipboard btn btn-xs btn-default" data-clipboard-text="<?= $row->no_rekening ?>" style="margin-left: 8px; padding: 3px 10px; background: #fff5f7; color: #e11d48; border: 1px solid #fecdd3; border-radius: 50px;">
                        <i class="fa fa-clipboard"></i> Salin
                    </button><br>
                    <small style="color: #881337;">a.n. <?= $row->nama_pemilik ?></small>
                    <?php if($row->qrcode_bank != '') { ?>
                    <div style="display:flex;align-items:center;justify-content:center;margin-top:12px;">
                        <img src="<?= base_url() ?>/assets/users/<?= $kunci; ?>/rekening/<?= $row->qrcode_bank ?>" alt="Qris" style="max-height:160px; border-radius: 12px; border: 1px solid #fecdd3;">
                    </div>
                    <?php } ?>
                </li>
                <?php } ?>
    ...
    <div id="ucapan-konten" class="konten" style="display: none;">
        <div class="card-sakura">
            ...
            <div class="form-group">
                <input id="nama" type="text" class="form-control" placeholder="Nama Anda" value="<?= esc($invite) ?>" style="background:#fff5f7; color:#4c0519; border:1px solid #fecdd3; border-radius:14px; padding:12px;" required>
            </div>
            <div class="form-group">
                <textarea id="komentar" class="form-control" placeholder="Tuliskan ucapan dan doa restu Anda..." rows="3" style="background:#fff5f7; color:#4c0519; border:1px solid #fecdd3; border-radius:14px; padding:12px;" required></textarea>
            </div>
            <button id="submitKomen" class="btn btn-block" style="background: linear-gradient(135deg, #fb7185, #e11d48); color:#fff; font-family:'Plus Jakarta Sans',sans-serif; font-weight:700; border-radius:50px; padding:14px; box-shadow: 0 8px 20px rgba(225,29,72,0.3);">
                Kirim Ucapan RSVP
            </button>
            <div id="loading_" style="display:none; text-align:center; margin-top:10px; color:#e11d48;">Mengirim ucapan...</div>

            <div class="layout-komen" style="margin-top: 25px;">
                <?php foreach($komen as $key => $data) { ?>
                <div class="komen" style="padding: 12px; border-bottom: 1px solid #ffe4e6; margin-bottom: 8px;">
                    <div style="font-weight: 700; color: #831843; font-size: 13px;"><?= \esc($data['nama_komentar']); ?></div>
                    <div style="font-size: 12px; color: #881337; margin-top: 4px;"><?= \esc($data['isi_komentar']); ?></div>
                </div>
                <?php } ?>
            </div>
        </div>
